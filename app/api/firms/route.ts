...
export async function GET(request: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      { type: "FeatureCollection", features: [], metadata: { filtered_detections: 0 } },
    );
  }
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { type: "FeatureCollection", features: [], metadata: { filtered_detections: 0, error: res.statusText } },
      );
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      let color = "#ffff00";
      let label = "Active";
      if (confNorm >= 90) { color = "#FF8C00"; label = "Stage 1"; }
      else if (confNorm >= 60) { color = "#ffff00"; label = "Medium"; }
      else { color = "#00C851"; label = "Low"; }
